...
<v-container>
	...
	<v-layout row wrap>
		...
		<select v-model="deptShow"  @change="changeDept($event)" class="form-control">
			...
			<v-flex md6 xs12>
			<date-picker type="year" placeholder="sy-start"  v-model="item.sy_start" valueType="format" lang="en" format="YYYY"></date-picker>	
		<date-picker type="year" placeholder="sy-end" v-model="item.sy_end" valueType="format" lang="en" format="YYYY"></date-picker>
		</v-flex>
	</v-layout>
	<v-layout row wrap>	
		<v-flex md6 xs7>
...
const department = [
				{
					...
					course:[
						{name:'Reseach Project',code:'ITPROJ'},
						{
